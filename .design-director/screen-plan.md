# Screen plan

- Site model: Corporate multipage.
- Selection screen: Home / first viewport.
- Selection viewport: 1440 x 900.
- Presentation mode: three separate composition frames, first-viewport-only.

| Route or frame | Purpose | Primary content | Primary action | Required proof |
|---|---|---|---|---|
| Home / first viewport | Position and convert | Value proposition, real transport image, hero route motif | Рассчитать перевозку | 20 лет, до 365 т, 500+ проектов |
| Home / services and projects | Explain capability | Service groups, selected projects, process | Смотреть услугу / проект | Real imagery and route stages |
| Service template | Match a transport need | Service scope, constraints, process, FAQ | Рассчитать перевозку | Relevant equipment and cases |
| Projects index | Demonstrate execution | Filterable or grouped project media | Открыть проект | Real cargo, route and date where verified |
| Project detail template | Deep proof | Challenge, solution, transport method, gallery | Обсудить похожую перевозку | Verified project facts |
| About / expertise | Establish company trust | History, fleet, team/process, documents | Связаться | 20 years, permits, certificates |
| Clients | Reinforce credibility | Real client logos and relationships | Рассчитать перевозку | Existing logo archive |
| Contacts | Complete inquiry | Phone, email, address, lead form | Отправить заявку | Same-origin form and consent |
| Privacy | Legal clarity | Personal-data policy | None | Current legal text |

## Composition selection contract

- Only the first row is visualized now.
- Remaining frames are expanded after composition and style selection.
- Existing public routes remain unchanged unless redirects are explicitly approved.
